...
var
  ...
  url: string;
begin
  // Crie um objeto TIdHTTP para fazer a solicitação HTTP à API do Banco Central do Brasil
  http := TIdHTTP.Create(nil);
  response := TStringStream.Create('');

  try
    // Faça a solicitação GET para a API do Banco Central do Brasil
    http.Get('https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoDolarDia', response);

    // Parseie a resposta JSON da API para obter a cotação do dólar
